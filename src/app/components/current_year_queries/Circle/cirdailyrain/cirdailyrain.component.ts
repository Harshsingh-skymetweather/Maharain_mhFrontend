...
@Component({
  selector: 'app-cirdailyrain',
  templateUrl: './cirdailyrain.component.html',
  styleUrls: ['./cirdailyrain.component.scss']
})
export class CirdailyrainComponent implements OnInit {
  ...
  this.divisions = res;
   ...
   onChangedivision(ev: any) {
    ...
    onSubmit() {
      ...
      {
       this.generateDates();
        this.service.circleDailyRain(this.selectedYear,this.selectedDivision,this.selectedDistrict,this.currentMonthIndex+1).subscribe((res: any[]) => {
        this.apiData = res;
