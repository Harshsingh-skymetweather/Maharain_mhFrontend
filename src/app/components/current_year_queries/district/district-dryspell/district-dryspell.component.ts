...
@Component({
  selector: 'app-district-dryspell',
  templateUrl: './district-dryspell.component.html',
  styleUrls: ['./district-dryspell.component.scss']
})
export class DistrictDryspellComponent implements OnInit {
  divisions:any;
  districts:any;
  statereports:any=[];
  report: any[] = [];
...
onSubmit() {
    if(this.selectedState == ''){
     ...
     {
      this.service.PDistrictDryspell(this.selectedYear,this.selectedDivision,this.selectedDistrict,this.fromDate,this.toDate,this.fromDuration,this.toDuration).subscribe((res) => {
      this.report = res;
